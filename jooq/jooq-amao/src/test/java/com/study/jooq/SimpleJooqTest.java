...
public class SimpleJooqTest {
    private final static Logger log = LoggerFactory.getLogger(SimpleJooqTest.class);
    ...
    @Test
    public void sunQuery() throws Exception {
        try (ScopedContext scopedContext = new ScopedContext()) {//try with resource
            DSLContext create = scopedContext.getDSLContext();
            //查询指定多个用户的最新一个订单信息
...
//            }
    ...
    @Test
    public void reuseStatement() throws Exception {
        //重复使用Statement
        try (ScopedContext scopedContext = new ScopedContext()) {//try with resource
            DSLContext create = scopedContext.getDSLContext();
